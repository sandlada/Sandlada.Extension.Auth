...
public sealed record RegistrationVerificationConstructorArgs {
...
public sealed class RegistrationVerification : IAggregate<Guid> {
    ...
    #region Constructors
    private RegistrationVerification() {
    }

    private RegistrationVerification(RegistrationVerificationConstructorArgs args) {
        this.Id = args.Id;
    ...
    public static IResult<RegistrationVerification> From(RegistrationVerificationConstructorArgs args) {
        if (string.IsNullOrWhiteSpace(args.VerificationCodeHash)) return Result.Failure<RegistrationVerification>(DomainError.Auth.InvalidVerificationChallenge);
        if (args.ExpiresAt <= args.CreatedAt) return Result.Failure<RegistrationVerification>(DomainError.Auth.InvalidVerificationChallenge);
        if (args.FailedAttemptCount < 0) return Result.Failure<RegistrationVerification>(DomainError.Auth.InvalidVerificationChallenge);
        if (args.RequestCount < 0) return Result.Failure<RegistrationVerification>(DomainError.Auth.InvalidVerificationChallenge);
        return Result.Success(new RegistrationVerification(args));
    }
    #endregion
    ...
    public IResult RegisterRequest(DateTime utcNow) {
        ...
        if (this.RequestCount >= 10) return Result.Failure(DomainError.Auth.RegistrationRequestLimitExceeded);
