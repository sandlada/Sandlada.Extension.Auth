...
public sealed record EmailRebindVerificationConstructorArgs {
    public required Guid Id { get; init; }
    public required Guid UserId { get; init; }
    public required EmailAddress TargetEmailAddress { get; init; }
...
public sealed class EmailRebindVerification : IAggregate<Guid> {

    #region Properties
    public Guid Id { get; private set; }
    public Guid UserId { get; private set; }
    public EmailAddress TargetEmailAddress { get; private set; } = default!;
    ...
    #region Constructors
    private EmailRebindVerification() {
    }

    private EmailRebindVerification(EmailRebindVerificationConstructorArgs args) {
        this.Id = args.Id;
        this.UserId = args.UserId;
        this.TargetEmailAddress = args.TargetEmailAddress;
    ...
    public static IResult<EmailRebindVerification> From(EmailRebindVerificationConstructorArgs args) {
        if (args.UserId == Guid.Empty) return Result.Failure<EmailRebindVerification>(DomainError.Auth.InvalidVerificationChallenge);
        if (string.IsNullOrWhiteSpace(args.VerificationCodeHash)) return Result.Failure<EmailRebindVerification>(DomainError.Auth.InvalidVerificationChallenge);
        if (args.ExpiresAt <= args.CreatedAt) return Result.Failure<EmailRebindVerification>(DomainError.Auth.InvalidVerificationChallenge);
        if (args.FailedAttemptCount < 0) return Result.Failure<EmailRebindVerification>(DomainError.Auth.InvalidVerificationChallenge);
        if (args.RequestCount < 0) return Result.Failure<EmailRebindVerification>(DomainError.Auth.InvalidVerificationChallenge);
        return Result.Success(new EmailRebindVerification(args));
    }
    #endregion
    ...
    public IResult RegisterRequest(DateTime utcNow) {
        ...
        if (this.RequestCount >= 10) return Result.Failure(DomainError.Auth.EmailRebindRequestLimitExceeded);
    ...
    public IResult Renew(EmailAddress targetEmailAddress, string verificationCodeHash, DateTime expiresAt, DateTime utcNow) {
